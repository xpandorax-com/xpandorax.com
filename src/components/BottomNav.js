...
export default function BottomNav() {
  ...
  const navItems = [
    { 
      ...
      label: 'Home', 
      icon: (active) => (
        <svg className={`w-6 h-6 ${active ? 'text-red-500' : ''}`} fill={active ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24">
    ...
    { 
      ...
      label: 'Search', 
      icon: (active) => (
        <svg className={`w-6 h-6 ${active ? 'text-red-500' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <nav className="bottom-nav md:hidden no-print" aria-label="Mobile navigation">
      <div className="flex items-center justify-around">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              {item.icon(active)}
              <span className={`text-[10px] mt-1 ${active ? 'text-red-500 font-medium' : 'text-gray-400'}`}>
                {item.label}
              </span>
            </Link>
          )
